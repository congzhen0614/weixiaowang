<template>
	<div class="periodical-ording" :class="{ topNeedPadding: topNeedPadding }">
		<!-- 购物球 -->
      ...
      <div v-for="ball in balls">
		...
		<keep-alive>
			<router-view :tabIndex="tabIndex" :listData="listData" v-if="$route.meta.keepAlive" @changeTabIndex="changeTabIndex" :title="title" :area="area" ref="home"></router-view>
		</keep-alive>
		<router-view :listData="listData" :area="area" v-if="!$route.meta.keepAlive"></router-view>
		...
		<p v-if="topNeedPadding" class="fixed_desc_text">尊敬的家长朋友！杂志订满150元包邮寄送；未满150元请拍运费差价25元，谢谢您的合作！</p>
	</div>
</template>
...
<script>
	...
	export default {
		...
		beforeRouteUpdate (to, from, next) {
			if (to.path === '/periodical/other1') {
				this.topNeedPadding = true
			} else {
				this.topNeedPadding = false
			}
			next(vm => {
			})
		},
		...
		methods: {
			...
			getData (type) {
				...
				this.$ajax.getAjax('/book/open?itemIds=1356,1367,1369,1715')
					.then(res => {
						// console.log(res)
						let list = res.data.pageInfo.list
						list = list.sort((first, next) => {
							return parseInt(first.id) - parseInt(next.id)
						})
						// 总页数
